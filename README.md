...
团支书可以管理本班通知、信息收集、会议纪要和知识资料；学生可以查看本班通知、提交信息与附件，并通过聊天框查询本班团务知识库。
...
- 团支书端和学生端ChatGPT式多轮聊天及SSE流式输出。
- 通知创建、发布、撤回、附件下载和学生已读统计。
- 动态信息收集、附件提交、退回重交和CSV导出。
- 文字、音频和视频会议材料处理，以及DeepSeek结构化会议纪要。
...
小块只用于精确检索，最终传给DeepSeek的是信息更完整的父块。同一父块被多个小块命中时只发送一次。
