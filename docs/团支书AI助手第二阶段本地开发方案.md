...
团支书和学生使用统一样式的 AI 对话中心，支持SQLite历史会话、多轮上下文、SSE流式输出、停止、错误重试和引用资料。每个账号只能查看自己的会话，页面展示完整历史，发送DeepSeek时只取最近10条成功消息。AI回答使用禁用原始HTML并经过消毒的Markdown渲染。会话不再区分“知识问答”和“会议整理”：团支书发送文字自动执行本班RAG，添加音视频自动启动会议Agent；学生只允许文字问答。通知管理中的AI起草保持独立。

全局绿色侧栏按身份生成导航。团支书的“班级工作台”下包含通知管理、信息收集、知识资料和会议文档；学生的“我的工作台”下包含本班通知和信息收集。小导航支持折叠并在当前登录期间保留状态，进入功能页时自动展开。“AI对话”是独立大入口，首页不再放置重复的AI卡片；手机端使用抽屉导航。
...
会议Agent使用LangGraph和独立SQLite Checkpointer。团支书在普通对话输入区添加最大500MB的单个音视频即可启动，也可同时填写补充要求；系统根据“团课、主题团日、支部会议”等关键词判断会议类型，无法判断时使用“其他”。流程为：上传音视频→FFmpeg提取音轨→faster-whisper本地转写→人工修改转写稿→本地脱敏→DeepSeek去口头语和重复内容→结构化纪要→人工修改确认→保存SQLite并生成DOCX。两个人工确认点通过LangGraph `interrupt()` 和 `Command(resume=...)` 暂停及恢复。确认后的完整纪要写入当前对话，支持继续追问；原始长转写稿不进入后续上下文。会议历史作为独立文档保留，删除对话不会删除已确认的会议文档。
