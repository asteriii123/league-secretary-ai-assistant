...
团支书创建、编辑、发布、撤回通知并查看阅读状态；学生查看本班已发布通知、下载附件并记录已读。

### 信息收集

团支书创建文本、日期、单选字段和附件任务，查看提交进度并退回；学生填写、上传、修改和重新提交。单文件默认限制20MB。

### 会议助手

支持粘贴文字稿，或使用FFmpeg和faster-whisper转写音视频。转写稿允许人工修改，脱敏后交给DeepSeek生成结构化会议纪要，人工确认后保存。
...
每阶段至少执行后端测试、前端类型检查和前端生产构建。第一阶段需验证数据库重启后仍存在、团支书和学生均能登录、学生邀请码有效、无令牌请求被拒绝、学生不能读取团支书邀请码。
